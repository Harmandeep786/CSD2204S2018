...
MariaDB [CSD2204S18]> SELECT COUNT(*),MANUFACTURER
    -> FROM products as p
    -> INNER JOIN manu m
    -> ON m.code=p.mnufacturer
    -> GROUP BY manufacturer;
ERROR 1054 (42S22): Unknown column 'p.mnufacturer' in 'on clause'
MariaDB [CSD2204S18]> SELECT COUNT(*),MANUFACTURER
    -> FROM products as p
    -> INNER JOIN manu m
    -> ON m.code=p.manufacturer
    -> GROUP BY manufcturer;
ERROR 1054 (42S22): Unknown column 'manufcturer' in 'group statement'
MariaDB [CSD2204S18]> SELECT COUNT(*),MANUFACTURER
    -> FROM products as p
    -> INNER JOIN manu m
    -> ON m.code=p.manufacturer
...
MariaDB [CSD2204S18]> SELECT manufacturer,m.name,COUNT(*) "no. of items produced" 
    -> from pproducts as p
    -> INNER JOIN manu m
    -> ON m.code=p.manufacturer
    -> GROUP BY manufacturer;
ERROR 1146 (42S02): Table 'csd2204s18.pproducts' doesn't exist
MariaDB [CSD2204S18]> SELECT manufacturer,m.name,COUNT(*) "no. of items produced" 
    -> from products as p
    -> INNER JOIN manu m
    -> ON m.code=p.manufacturer
...
MariaDB [CSD2204S18]> SELECT manufacturer,m.name,COUNT(*) "no. of items produced" 
    ...
    -> INNER JOIN manu m
    -> ON m.code=p.manufacturer
    -> WHERE m.code <4 GROUP BY manufacturer;
+--------------+-----------------+-----------------------+
| manufacturer | name            | no. of items produced |
+--------------+-----------------+-----------------------+
|            1 | Sony            |                     1 |
...
MariaDB [CSD2204S18]> SELECT PRODUCTS.CODE,PROUCTS.NAME,MANU.NAME
    -> FROM MANU
    -> LEFT JOIN PRODUCTS
    -> ON PRODUCTS.CODE=MANU.CODE;
ERROR 1054 (42S22): Unknown column 'PROUCTS.NAME' in 'field list'
MariaDB [CSD2204S18]> SELECT PRODUCTS.CODE,PRODUCTS.NAME,MANU.NAME
    -> FROM MANU
    -> LEFT JOIN PRODUCTS
    -> ON PRODUCTS.CODE=MANU.CODE;
